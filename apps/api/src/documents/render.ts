...
<div class="wrap">
  ...
  <p class="cl2"><b>2.3.18.</b>Своевременно предоставлять Скутер Арендодателю для планового обслуживания (замены масла) согласно межсервисному интервалу, указанному на сервисном стикере Скутера. В случае несвоевременной замены масла (превышения указанного на сервисном стикере пробега замены) Арендатор уплачивает Арендодателю штраф в размере 5 000 (пять тысяч) рублей 00 копеек.</p>
  <p class="cl2"><b>2.3.19.</b>Вернуть Скутер с тем же уровнем топлива, с которым Скутер был выдан Арендатору. Уровень топлива на момент выдачи фиксируется в видео-акте передачи Скутера. При возврате Скутера с уровнем топлива, не соответствующим уровню при выдаче, Арендатор уплачивает Арендодателю компенсацию в размере 350 (триста пятьдесят) рублей 00 копеек.</p>
  ...
  <p class="cl"><b>4.3.</b>Если настоящим Договором не предусмотрено иное, Стороны договариваются о возмещении ущерба исходя из цен на запасные части, работы и услуги согласно прайсу, установленному Арендодателем. Прайс предоставляется Арендатору для ознакомления по его запросу. При отсутствии соответствующей позиции в прайсе применяется рыночная стоимость.</p>
  ...
  <p class="cl"><b>6.4.</b>По желанию Арендатора договор может быть досрочно расторгнут — в том числе если Арендатор передумал пользоваться Скутером либо решил вернуть Скутер раньше установленного срока. При этом перерасчёт стоимости аренды не производится, и сумма, уплаченная за весь срок аренды по данному договору, возврату не подлежит.</p>
  ...
  <p class="cl"><b>7.3.</b>Скутер может быть возвращён до окончания срока, указанного в пункте 3.1.2. настоящего договора, только при условии уведомления Арендодателя не менее чем за сутки. При этом стоимость арендной платы, указанная в п. 3.3. настоящего Договора, возврату не подлежит; перерасчёт стоимости аренды при досрочном возврате не производится.</p>
...
function tplContractEbike(b: Bundle): string {
  ...
  // Пункты 2.3.18/2.3.19 применимы только к мототехнике (масло, топливо).
  // Заменяем их электрическими аналогами НА ИХ МЕСТАХ — нумерация
  // договора не сдвигается.
  html = replaceClause(
    html,
    "2.3.18.",
    "Заряжать Электровелосипед только штатным зарядным устройством, переданным по Акту приёма-передачи, и не оставлять аккумулятор на зарядке без присмотра. Запрещается вскрывать, разбирать или самостоятельно ремонтировать аккумуляторную батарею.",
  );
  html = replaceClause(
    html,
    "2.3.19.",
    "Вернуть Электровелосипед с заряженным аккумулятором — не ниже уровня заряда на момент выдачи. Уровень заряда при выдаче фиксируется в видео-акте передачи. При возврате с уровнем заряда ниже выданного Арендатор уплачивает Арендодателю компенсацию в размере 350 (триста пятьдесят) рублей 00 копеек.",
  );
