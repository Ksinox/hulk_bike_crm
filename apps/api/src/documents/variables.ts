...
export function substituteVariables(html: string, b: Bundle): string {
  ...
  // выдан: [pill]. Код подразделения [pill].». После подстановки плашек
  // (которые для иностранца возвращают пустую строку) остаётся текст с
  // подписями без значений — заменяем весь параграф на passportRaw.
  //
  // Регекс жадно ловит «Паспорт серия» вплоть до «Код подразделения [...].»
  // Любая комбинация подписей и пустых пилюль между ними корректно
  // съедается из-за `[\s\S]*?`. Если в шаблоне нет «Код подразделения» —
  // регекс не сработает, шаблон останется как есть (это допустимо).
  if (b.client.isForeigner) {
    const raw = (b.client.passportRaw ?? "").trim();
    out = out.replace(
      /Паспорт\s+серия[\s\S]*?Код\s+подразделения[^.]*\.?/gi,
