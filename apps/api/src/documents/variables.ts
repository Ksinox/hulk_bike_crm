...
export function resolveVariable(key: string, b: Bundle): string {
  ...
  if (key.startsWith("client.")) {
    ...
    // Для иностранного гражданина РФ-паспортные плашки пустые. Без этого
    // в overrides, сделанных под РФ-формат («Паспорт серия [pill] номер
    // [pill]. Дата выдачи [pill]. Кем выдан: [pill]. Код подразделения
    // [pill]»), вместо данных подставлялись прочерки. Сейчас плашки
    // возвращают пустую строку, а сам РФ-параграф мы потом коллапсируем
    // в substituteVariables: целиком заменяем на passportRaw (одна
    // свободная строка, как клиент ввёл при редактировании).
...
export function substituteVariables(html: string, b: Bundle): string {
  ...
  // Иностранный гражданин: коллапсируем РФ-параграф паспорта в одну
  // свободную строку passportRaw. Override-шаблоны под РФ-формат сделаны
  // как «Паспорт серия [pill] номер [pill]. Дата выдачи [pill]. Кем
  // выдан: [pill]. Код подразделения [pill].». Для иностранца плашки
  // возвращают пустую строку — остаётся «Паспорт серия  номер . Дата
  // выдачи . Кем выдан: . Код подразделения .» — заменяем весь блок
  // на passportRaw.
  //
  // Регекс СТРОГИЙ: между «серия» и «номер» только whitespace (т.е. блок
  // с пустыми значениями), и между остальными подписями — только пробелы
  // и точки. Это не ловит landlord-блок, где между «серия» и «номер»
  // стоит реальный номер серии (например, «серия 0313 номер 294328»).
  // Раньше менее строгий регекс портил данные арендодателя — заменял
  // его паспортный блок на passportRaw клиента.
  if (b.client.isForeigner) {
    const raw = (b.client.passportRaw ?? "").trim();
    out = out.replace(
      /Паспорт\s+серия\s+номер\s*\.\s*Дата\s+выдачи\s*\.\s*Кем\s+выдан:?\s*\.\s*Код\s+подразделения\s*\.?/gi,
      raw ? escapeForHtml(raw) + "." : "",
    );
  }
...
export function replaceFixtureWithPlaceholders(html: string): string {
  ...
  // мы их вычищали чтобы не было дубля «Дата рождения [Дата рождения]» в
  // редакторе, но это ломало финальный документ: после подстановки
  // оставались голые значения подряд («14.02.2000. 4520 112358. ...»),
  // и пользователь не понимал что есть что. Сохраняем семантику в
  // шаблоне — дубль в редакторе небольшая UX-плата за читаемый
  // документ. Пользователь при желании может удалить дубль вручную.
