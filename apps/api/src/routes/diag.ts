...
export async function diagRoutes(app: FastifyInstance) {
  ...
  // GET /api/_diag/backup-download вынесён в публичный backupPublicRoutes
  // (apps/api/src/routes/backup-public.ts) — иначе глобальный requireAuth
  // под protectedApp возвращал 401 GitHub Action'у без cookie.
}
