...
/**
 * Окно фиксации замены масла.
 *
 * Замена масла — это СОБЫТИЕ обслуживания, а не расход: важно только на каком
 * пробеге и когда меняли. От введённого пробега ведётся счёт до следующей
 * замены (интервал 3000 км) — чтобы по парку было видно, у какого скутера
 * масло скоро менять, а у какого ещё свежее. Стоимость не спрашиваем.
 *
 * Делает два действия:
 *   1) создаёт запись обслуживания kind:"oil" (для истории замен — видно ДАТУ
 *      и пробег каждой замены), сумма = 0 (не расход);
 *   2) обновляет lastOilChangeMileage = введённый пробег (точка отсчёта
 *      интервала; серверный роут scooter-maintenance это поле НЕ трогает).
 *
 * Подходит и для «только что поменял» (пробег ≈ текущий, дата сегодня), и для
 * «прошлая замена была давно» (меньший пробег / прошлая дата) — для скутеров,
 * у которых история ещё не велась.
 */
export function OilChangeDialog({
  ...
  // защита от опечатки (260000 вместо 26000). Если масло реально меняли на
  // большем пробеге — сначала обновите пробег скутера в карточке.
  ...
  // Следующая замена «при N км» от введённой точки — для подсказки.
  const nextAtKm = mileageValid ? mileageNum + OIL_INTERVAL_DEFAULT_KM : null;

  const submit = async () => {
    if (!canSave) return;
    try {
      // 1) запись обслуживания (событие, без стоимости).
      await createMaint.mutateAsync({
        scooterId,
        kind: "oil",
        performedOn,
        amount: 0, // замена масла — событие, не расход
        mileage: mileageNum,
        note: note.trim() ? note.trim() : null,
      });
      // 2) точка отсчёта интервала на скутере.
      ...
      >
        ...
        <div className="flex flex-col gap-4 px-5 py-4">
          <p className="text-[12px] leading-snug text-muted">
            Просто фиксируем событие: на каком пробеге меняли масло. От этого
            пробега ведём счёт до следующей замены (через{" "}
            {fmtKm(OIL_INTERVAL_DEFAULT_KM)} км). Стоимость указывать не нужно.
          </p>
          ...
          <div>
            ...
            {!mileageValid ? (
              ...
              <div className="mt-1 text-[11px] text-muted">
                Следующая замена — при {fmtKm(nextAtKm)} км.
              </div>
            ) : null}
          </div>
