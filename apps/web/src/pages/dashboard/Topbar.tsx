      ...
      {newRentalOpen && (
        <NewRentalModal
          ...
          onCreated={(r) => {
            ...
            // v0.9.1: «принцип одного экрана». Закрываем drawer-стек И
            // переходим на страницу аренды с новой связкой (как из
            // паркинга/скутера). Раньше openRental открывал карточку в
            // drawer ПОВЕРХ уже открытой карточки страницы (на вкладке
            // Аренды) — получались две карточки рядом. navigate заменяет
            // выбранную аренду на странице → ровно одна карточка.
            drawer.close();
