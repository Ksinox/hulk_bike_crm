...
export function DocumentPreviewModal({
  ...
  const handlePrint = () => {
    ...
    // Авто-снапшот при печати: фиксируем именно ту версию документа,
    // которую оператор вывел на бумагу. Это и есть «архив подписанной
    // версии» — даже если данные клиента/скутера потом изменятся, можно
    // открыть точно тот же рендер. Если оператор печатает ещё раз чуть
    // позже — будет второй снапшот, лишние можно удалить кнопкой
    // «Удалить» в карточке аренды (вкладка «Документы»).
    //
    // Запускаем не блокирующе: ошибка сохранения не должна ломать саму
    // печать, юзер видит только тост (success или error).
    if (rentalId && documentType) {
      saveSnapshot
        .mutateAsync({ rentalId, type: documentType })
        .then((snap) => {
          toast.success(
            "Версия документа сохранена",
            `«${snap.title}» доступна в карточке аренды → «Документы» → «Сохранённые версии».`,
          );
        })
        .catch((e: unknown) => {
          // Тихо логируем — печать важнее автосейва, не пугаем юзера.
          // eslint-disable-next-line no-console
          console.error("Auto-save on print failed:", e);
          toast.error(
            "Не удалось сохранить версию",
            "Печать прошла, но автосохранение не сработало. Можно нажать «Сохранить» руками.",
          );
        });
    }
  };
