...
С 2026-06-01 у CRM есть полноценная **мобильная версия** (отдельный слой `apps/web/src/mobile/`, развилка по `useIsMobile` в `App.tsx`: <768px или планшет — основной указатель палец, при любой ширине; раскладка планшета — `mobile/tablet.ts`). Десктоп и мобилка живут в одной кодовой базе.
...
### ЖЕЛЕЗНОЕ ПРАВИЛО приёмки (с 2026-08-23)

Работа ведётся по бэклогу [`ROADMAP.md`](ROADMAP.md), у каждого пункта **две галочки**:

1. **«Сделано»** ставит разработка — но только после того, как **сама зашла на preview через MCP-браузер**, прокликала сценарий, сняла скриншоты и убедилась, что работает. Не «задеплоил и написал готово», а **проверил вживую своими глазами**.
2. **«Принято»** ставит ТОЛЬКО заказчик после личной проверки.

**В `main` (прод) уходит исключительно то, что имеет обе галочки.** Если в работе пять пунктов, а принято два — мержим только эти два. Поэтому каждый пункт делаем **отдельной веткой от `main`**; preview собирает всё, что в работе, `main` получает только принятое.

**UX/UI — часть задачи, не довесок.** Делать компактно и удобно ЧЕЛОВЕКУ: не нагромождать, вписывать в существующие блоки, а не плодить новые. Проверять глазами на скриншоте, что вёрстка не поехала (см. авто-память `verify_layout_not_just_presence`).

**Копим «было/стало».** По каждому пункту снимаем скриншот ДО правки и ПОСЛЕ — из них в конце собираем HTML-отчёт заказчику (проблема → решение, новые функции — как мини-туториал). Пока релиз не смержен, прод = «было», preview = «стало».
